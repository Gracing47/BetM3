...
export const Web3Provider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const acceptBet = useCallback(async (
    ...
    const contract = getNoLossBetContract();
    const stakeAmount = customStake ? ethers.utils.parseEther(customStake) : ethers.BigNumber.from(0);
    ...
    // Call the contract with the appropriate parameters
    if (customStake && commentText) {
      return await contract.acceptBet(betId, prediction, stakeAmount, commentText);
    } else if (customStake) {
      return await contract.acceptBet(betId, prediction, stakeAmount, "");
    } else if (commentText) {
      return await contract.acceptBet(betId, prediction, ethers.BigNumber.from(0), commentText);
    } else {
      return await contract.acceptBet(betId, prediction, ethers.BigNumber.from(0), "");
    }
  }, [signer, getNoLossBetContract]);
