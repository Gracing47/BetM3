...
async function main() {
  try {
    ...
    // Add 50% buffer to ensure we're above the minimum and can replace pending tx
    ...
    // Send a transaction to self with a higher gas price to clear pending transactions
    console.log("Sending transaction to clear pending transactions...");
    const tx = await deployer.sendTransaction({
      to: deployerAddress,
      value: 0,
      gasPrice: gasPriceWithBuffer,
      gasLimit: 21000, // Standard gas limit for a simple transfer
      nonce: currentNonce - 1 // Use the nonce of the pending transaction
    });
    ...
    console.error("Failed to clear pending transactions!");
